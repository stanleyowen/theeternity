...
$targetDir 		= "../upload-data/ig-proof-2/";
$fileName 		= basename($_FILES["file"]["name"]);
$fileSize 		= basename($_FILES["file"]["size"]);
$targetFilePath = $targetDir . $fileName;
$fileType 		= pathinfo($targetFilePath,PATHINFO_EXTENSION);

if(isset($_COOKIE['ULD-129'])){
		...
		if (($fileSize > 557150)){
			...
			if(in_array($fileType, $allowTypes)){
				if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)){
					$insert = $connect->query("UPDATE scrim_member SET ig_proof_2='".$fileName."' WHERE username='$username'");
					if($insert){
						echo '<script>alert("Photo Uploaded Successfully.");window.history.back();</script>';
						setcookie('ULD-129',sha1(md5("Succeed")),time()+(15*8400),'/',"000webhostapp.com",isset($_SERVER["HTTPS"]), true);
					}else{
