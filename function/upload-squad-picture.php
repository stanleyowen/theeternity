...
$targetDir 		= "../upload-data/logo-squad/";
$fileName 		= basename($_FILES["file"]["name"]);
$fileSize 		= basename($_FILES["file"]["size"]);
$targetFilePath = $targetDir . $fileName;
$fileType 		= pathinfo($targetFilePath,PATHINFO_EXTENSION);

if(isset($_COOKIE['ULD-014'])){
	echo '<script>alert("Each User can ONLY upload ONCE");window.history.back();</script>';
}else {
	if(isset($_POST["submit"]) && !empty($_FILES["file"]["name"])){
		if (($fileSize > 557150)){
			echo '<script>alert("File size too large. Make sure file size under 500kb");window.history.back();</script>';
		}else {
			$allowTypes = array('jpg','jpeg','png');
			if(in_array($fileType, $allowTypes)){
				if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)){
					$insert = $connect->query("UPDATE scrim_member SET squad_pic='".$fileName."' WHERE username='$username'");
					if($insert){
						echo '<script>alert("Photo Uploaded Successfully.");window.history.back();</script>';
						setcookie('ULD-014',sha1(md5("Uploaded")),time()+(15*8400),'/',"000webhostapp.com",isset($_SERVER["HTTPS"]), true);
					}else{
